...
Core tests take longer than any other IGWT tests, due to the high amount of computational work involved in validating core components.
...
To run only IGWT's core tests (after building):
...
To run only IGWT's crypto tests (after building):
...
```
igwtd --regtest --offline --fixed-difficulty 1
```
...
To run only IGWT's hash tests (after building):
...
To run only IGWT's performance tests (after building):
...
To run only IGWT's unit tests (after building):
